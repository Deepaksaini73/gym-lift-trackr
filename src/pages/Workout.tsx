...
const Workout = () => {
  ...
  const loadTodayWorkouts = () => {
    ...
    try {
      ...
      const logs = getWorkoutLogs()
        .filter((log) => log.user_id === currentUserId && log.date === today)
  ...
  const addNewSet = () => {
    setEditSets([...editSets, { reps: 0, weight: 0 }]);
  };
  ...
  const handleUpdate = () => {
    ...
    try {
      ...
      updateWorkoutLog(editingWorkout.id, {
        sets: validSets,
                  ...
                  <Card key={workout.id} className="p-4 bg-card border-border hover:border-primary/50 transition-all">
                    ...
                    <div className="space-y-2">
                      ...
                      {workout.sets.map((set, idx) => (
                        <div key={idx} className="grid grid-cols-3 gap-2 text-sm bg-muted/30 rounded p-2">
      ...
      <Dialog open={!!editingWorkout} onOpenChange={() => setEditingWorkout(null)}>
        <DialogContent className="sm:max-w-[500px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{editingWorkout?.exercise_name}</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            {editSets.map((set, index) => (
              <div key={index} className="grid grid-cols-[60px_1fr_1fr_50px] gap-2 items-center">
                <span className="text-sm font-medium text-foreground flex items-center">{index + 1}</span>
                <Input
                  type="number"
                  value={set.reps || ""}
